...
public class IsCompletedByCharityFunction implements RequestHandler<RequestClass, ResponseClass> {
    @Override
    public ResponseClass handleRequest(RequestClass input, Context context) {
        ...
        try {
            ...
            Boolean isCompletedByCharity = true;
            DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
            Date dateObject = new Date();

            String lastUpdateDate = dateFormat.format(dateObject);
            String currentStatus = "";
            if(item.getBoolean("isCompletedByUser") == true){
                currentStatus = "Confirmed By Both";
            }else {
                currentStatus = "Confirmed By Charity and Pending By User";
            }

            UpdateItemOutcome outcome = table.updateItem("id", id,
                    new AttributeUpdate("isCompletedByCharity").put(isCompletedByCharity),
